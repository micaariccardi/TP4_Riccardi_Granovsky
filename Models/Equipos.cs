...
{
    public static List<string> listaEquipos {get; private set;} = new List<string> {"Boca","River", "San Lorenzo", "Independiente", "Racing", "Defensa y Justicia", "Huracán", "Argentina Jrs.", "Banfield", "Veléz"};
    ...
    public static Dictionary <string, Indumentaria> IndumentariaEquipos {get; private set;} = new Dictionary<string, Indumentaria>();

    public static bool IngresarIndumentaria(string equipoSeleccionado, Indumentaria item)
    {
        
        if(!IndumentariaEquipos.ContainsKey(equipoSeleccionado))
        {
            IndumentariaEquipos.Add(equipoSeleccionado, item);
            return true;
        }
        return false;
    }
